...
class IndianStockDataFetcher:
    ...
    def get_nse_data_alternative(self, symbol):
    ...
    def get_nse_data_fallback(self, symbol):
    ...
    def get_bse_data_alternative(self, scrip_code):
    ...
    def get_all_data_yfinance(self, symbols):
        """Get all data using yfinance (most reliable)

        symbols: list of ticker strings to try in order.
        """
        try:
            # Try provided symbols
            if not symbols:
    ...
    def display_data(self, data, company_name=None):
        """Display data in formatted table"""
        if not data:
            print("No data to display")
            return
        
        company = company_name or data.get('Company Name', 'STOCK DATA')
        print("\n" + "="*80)
        print(f"{company.upper()} - COMPLETE STOCK DATA")
        print("="*80)
...
def main(input_file=None, output_file=None):
    ...
    Args:
        input_file: Path to stocks JSON config (default: 'input/stocks_list.json')
        output_file: Path to output CSV file (default: 'output/all_stocks_data.csv')
    """
    input_path = input_file or os.path.join('input', 'single_stock.json')
    output_path = output_file or os.path.join('output', 'single_stock.csv')
    
    print(f"Reading stocks from: {input_path}")
    print("="*80)
    ...
    for stock in stocks_list:
        ...
        if data:
            ...
            fetcher.display_data(data, company_name=name)
            fetcher.save_to_csv(data, filename=output_path)
        else:

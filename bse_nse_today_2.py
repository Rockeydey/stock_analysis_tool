...
class IndianStockDataFetcher:
    ...
    def get_all_data_yfinance(self):
        """Get all data using yfinance (most reliable)"""
        try:
            # Try different symbols
            symbols = ["TARIL.NS", "TRANSFOR.NS", "532928.BO"]
...
def main():
    ...
    print("\nMethod 1: Using Yahoo Finance (Most Reliable)...")
    ...
    if data:
        ...
        print("\nMethod 2: Trying NSE API...")
        nse_data = fetcher.get_nse_data_alternative("TRANSFOR")
        if nse_data:
            print("NSE Data:")
            for key, value in nse_data.items():
                print(f"  {key}: {value}")
        
        print("\nMethod 3: Trying BSE API...")
        bse_data = fetcher.get_bse_data_alternative("532928")
        if bse_data:
            print("BSE Data:")
            for key, value in bse_data.items():
                print(f"  {key}: {value}")
        
        print("\nMethod 4: Getting Financial Ratios...")
        financial_data = fetcher.get_financial_data()
        if financial_data:
